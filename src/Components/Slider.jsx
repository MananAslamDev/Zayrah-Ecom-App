...
const ProductSlider = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const productData = response.data.products.slice(0, 10).map((product) => ({
          id: product.id,
          title: product.title,
          image: product.image,
          price: product.price,
  ...
  return (
    <div className="w-full max-w-7xl mx-auto py-10 text-center">
      <h2 className="text-3xl font-bold text-gray-900 mb-6">Featured Headphones</h2>
      <Swiper {...swiperSettings}>
        {products.map((product) => (
          <SwiperSlide key={product.id} className="p-5">
            <div className="bg-white text-gray-900 rounded-lg shadow-md transform transition hover:scale-105 p-5 h-[350px] flex flex-col">
              <img
                className="w-full h-[200px] object-contain rounded-md mb-3"
                src={product.image}
                alt={product.title}
              />
              <h3 className="text-sm font-semibold text-gray-700 line-clamp-2 h-10 mb-2">{product.title}</h3>
